...
# IMPORTANT: Change this to your actual deployed Streamlit Cloud URL after deployment
redirect_uri = st.sidebar.text_input(
    "Redirect URI (change this after deployment)",
    value="https://your-app-name.streamlit.app",  # ← replace with your real URL
    help="After you deploy, copy the live URL from Streamlit and paste it here"
)
...
if client_id and client_secret and st.sidebar.button("🔑 Connect to Xero"):
    auth_url = (
        ...
        f"&scope=accounting.transactions offline_access&state=12345"
    )
    st.sidebar.markdown(f"[1. Click here to log into Xero →]({auth_url})")
    st.sidebar.info("2. After login, copy the FULL browser URL and paste it below")
...
if auth_code and client_id and client_secret and st.sidebar.button("Exchange for Token"):
    ...
    if response.status_code == 200:
        ...
        st.sidebar.error("Token exchange failed – double-check the code")

# ====================== UPLOAD & FREE GEMINI OCR ======================
uploaded_file = st.file_uploader("Upload invoice / bill (PDF or image)", type=["pdf", "png", "jpg", "jpeg"])

if uploaded_file and gemini_key and st.session_state.get("xero_token"):
...
st.caption("100% free • Gemini OCR • Direct to Xero • Deployed on Streamlit Community Cloud")
